...
An example file is provided in /docker/kinetica-cluster/resources/gpudb.conf.template for kinetica version 6.2.0.15 - all you need to do is:
...
* head_ip_address = 172.19.0.10 # static IP of kinetica-6.2-head docker container
* enable_worker_http_servers = true # turn on multi-head ingest support
* rank0.host = 172.19.0.10 # static IP of kinetica-6.2-head docker container
* rank1.host = 172.19.0.10 # static IP of kinetica-6.2-head docker container
* rank2.host = 172.19.0.11 # static IP of kinetica-6.2-worker docker container
...
This creates a docker image called apwynne/kinetica-6.2-node. confirm as follows:

	$ docker image ls | grep kinetica-6.2-node
...
This will start 2 docker containers to form a small Kinetica cluster; the containers are called kinetica-6.2-head and kinetica-6.2-worker. These are given fixed IP addresses which correspond to the expected IP addresses in the gpudb.conf script as described above.
...
Now edit the api/pom.xml to give the connection details for your Kinetica server. Look for the line:
	    ...
	    <argLine>-DbeamTestPipelineOptions='["--kineticaURL=http://kinetica-6.2-head:9191", "--kineticaUsername=admin", "--kineticaPassword=Kinetica1!", "--kineticaTable=scientists", "--runner=DirectRunner"]'</argLine>
...
check Kinetica is listening as follows:
	...
	# curl http://`getent hosts kinetica-6.2-head | cut -d ' ' -f1`:9191
	Kinetica is running!
...
Now run the beam job:
	...
	# /opt/spark-2.3.1-bin-hadoop2.7/bin/spark-submit --master spark://`getent hosts spark-cluster | cut -d ' ' -f1`:7077 --conf spark.executor.memory=2G --class com.kinetica.beam.example.Test1 apache-beam-kineticaio-example-1.0-shaded.jar --kineticaPassword=Kinetica1! --kineticaTable=scientists --kineticaURL=http://`getent hosts kinetica-6.2-head | cut -d ' ' -f1`:9191 --kineticaUsername=admin --runner=SparkRunner
